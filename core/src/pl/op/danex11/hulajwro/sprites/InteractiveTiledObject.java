...
public abstract class InteractiveTiledObject {
    ...
    public InteractiveTiledObject(World world, TiledMap map, Rectangle bounds){
        ...
        bdef.position.set((bounds.getX() + bounds.getWidth() / 2)/ SpielFahre.PPM, (bounds.getY() + bounds.getHeight() / 2)/ SpielFahre.PPM);
        body = world.createBody(bdef);
        shape.setAsBox((bounds.getWidth() / 2)/ SpielFahre.PPM, (bounds.getHeight() / 2)/ SpielFahre.PPM);
        fdef.shape = shape;
